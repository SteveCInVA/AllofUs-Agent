...
- **Classification + group IDs are config, not code.** Each dataset's
  ...
  name come from **app settings** (GCC-tenant-specific and operationally adjustable).
  A **`DATASET_CLASSIFICATION` app-settings JSON map** drives per-dataset
  classification; `BASE_ENTITLEMENT_GROUP_ID` and `ADMIN_ROLE=Agent.Admin` cover the
  base/admin entitlements. **Adjusting classification requires an app-settings change
  (which restarts the app) — never an application redeploy.**
...
- **Classification is config-driven via app settings.** A single
  `DATASET_CLASSIFICATION` **app-settings JSON** map is the source of truth:
...
- **No redeploy to change classification.** Because it lives in app settings, flipping
  a dataset public↔restricted or changing its group is an **app-settings update (which
  restarts the app)**, plus the matching Entra group work. It is **not** an application
...
## 6. Open decisions
1. **Classification config channel:** ✅ **DECIDED — app-settings JSON**
